Feature: SQL Maria DB

  # Maria DB will be transformed correctly.
  @mariadb
  Scenario Outline: Transform
    Given mariadb.<pUnique> the input file <pInputFile>
    And mariadb.<pUnique> the output file <pOutputFile>
    When mariadb.<pUnique> the values are passed into Cimerant
    Then mariadb.<pUnique> Cimerant outputs

    Examples:
      | pUnique | pInputFile             | pOutputFile        |
      | 1       | "fast/ddl_create.sql"  | "fast/ddl_create"  |
      | 2       | "fast/ext_tests.sql"   | "fast/ext_tests"   |
  ...
  Scenario Outline: TransformShared
    ...
    When mariadb.<pUnique> the shared values are passed into Cimerant
    Then mariadb.<pUnique> Cimerant shared outputs
