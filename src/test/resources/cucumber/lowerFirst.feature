Feature: Lower First
  ...
  # The user will be able to combine a Velocity template and a formatted file to transform the values to lower first.
  @lowerFirstTemplate
  Scenario Outline: Template
    Given lowerFirstTemplate.<pUnique> input file <pInputFile>
    * lowerFirstTemplate.<pUnique> template <pTemplate>
    * lowerFirstTemplate.<pUnique> templates <pTemplates>
    * lowerFirstTemplate.<pUnique> single multi value <pSinti>
    * lowerFirstTemplate.<pUnique> path <pPath>
    * lowerFirstTemplate.<pUnique> file <pFile>
    * lowerFirstTemplate.<pUnique> var <pVar>
    * lowerFirstTemplate.<pUnique> vars <pVars>
    When lowerFirstTemplate.<pUnique> the values are passed into Cimerant
    Then lowerFirstTemplate.<pUnique> values are converted according to the template
  ...
  # The user will be able to transform values to lower first.
  @lowerFirstValue
  Scenario Outline: Value
    Given lowerFirstValue.<pUnique> the input <pInputString> is passed into Cimerant
    Then lowerFirstValue.<pUnique> Cimerant outputs <pOutputString>
