Feature: Command Line Arguments
  ...
  Scenario Outline: commandLineArguments
    Given commandLineArguments.<pUnique> JSON file to be transformed <pInputFile>
    * commandLineArguments.<pUnique> template to use to transform the JSON <pTemplate>
    * commandLineArguments.<pUnique> a list of templates to use to transform the JSON <pTemplates>
    * commandLineArguments.<pUnique> single multi value <pSinti>
    * commandLineArguments.<pUnique> path <pPath>
    * commandLineArguments.<pUnique> file <pFile>
    * commandLineArguments.<pUnique> var <pVar>
    * commandLineArguments.<pUnique> vars <pVars>
    When commandLineArguments.<pUnique> the values are passed into Cimerant
    Then commandLineArguments.<pUnique> an identical file is produced
