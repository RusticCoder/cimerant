Feature: Upper Case
  ...
  # The user will be able to combine a Velocity template and a formatted file to transform the values to upper case.
  @upperCaseTemplate
  Scenario Outline: Template
    Given upperCaseTemplate.<pUnique> input file <pInputFile>
    * upperCaseTemplate.<pUnique> template <pTemplate>
    * upperCaseTemplate.<pUnique> templates <pTemplates>
    * upperCaseTemplate.<pUnique> single multi value <pSinti>
    * upperCaseTemplate.<pUnique> path <pPath>
    * upperCaseTemplate.<pUnique> file <pFile>
    * upperCaseTemplate.<pUnique> var <pVar>
    * upperCaseTemplate.<pUnique> vars <pVars>
    When upperCaseTemplate.<pUnique> the values are passed into Cimerant
    Then upperCaseTemplate.<pUnique> values are converted according to the template
  ...
  # The user will be able to transform values to upper case.
  @upperCaseValue
  Scenario Outline: Value
    Given upperCaseValue.<pUnique> the input <pInputString> is passed into Cimerant
    Then upperCaseValue.<pUnique> Cimerant outputs <pOutputString>
