Feature: Dot Lower Case

  # The user will be able to combine a Velocity template and a formatted file to transform the values to dot lower case.
  @dotLowerCaseTemplate
  Scenario Outline: Template
    Given dotLowerCaseTemplate.<pUnique> input file <pInputFile>
    * dotLowerCaseTemplate.<pUnique> template <pTemplate>
    * dotLowerCaseTemplate.<pUnique> templates <pTemplates>
    * dotLowerCaseTemplate.<pUnique> single multi value <pSinti>
    * dotLowerCaseTemplate.<pUnique> path <pPath>
    * dotLowerCaseTemplate.<pUnique> file <pFile>
    * dotLowerCaseTemplate.<pUnique> var <pVar>
    * dotLowerCaseTemplate.<pUnique> vars <pVars>
    When dotLowerCaseTemplate.<pUnique> the values are passed into Cimerant
    Then dotLowerCaseTemplate.<pUnique> values are converted according to the template
  ...
  # The user will be able to transform values to dot lower case.
  @dotLowerCaseValue
  Scenario Outline: Value
    Given dotLowerCaseValue.<pUnique> the input <pInputString> is passed into Cimerant
    Then dotLowerCaseValue.<pUnique> Cimerant outputs <pOutputString>
