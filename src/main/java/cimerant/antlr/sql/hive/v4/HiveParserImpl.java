...
import sql.hive.v4.HiveLexer;
import sql.hive.v4.HiveParser;
import sql.hive.v4.HiveParserListener;

/** This is all the parsing support code essentially; most of it is error recovery stuff. */
public class HiveParserImpl extends HiveParser {
  private static final CimerantLogger logger;

  static {
    logger = (CimerantLogger) LoggerFactory.getLogger(HiveParserImpl.class.getName());
  }
   ...
   */
  public static HiveParserImpl getInstance(final File file) throws IOException {
    final var moduleCode = ModuleCode.ERR_M2500;

    try {
      final var lexer = new HiveLexer(CharStreams.fromFileName(file.getCanonicalPath()));
      lexer.removeErrorListeners();
      lexer.addErrorListener(new ProxyErrorListenerImpl(lexer.getErrorListeners()));

      return new HiveParserImpl(new CommonTokenStream(lexer));
    } catch (final SysError s) {
      throw s;
    } catch (final Throwable t) {
      // 0001 | Unknown error
      if (HiveParserImpl.logger.isDebugEnabled()) {
        HiveParserImpl.logger.debug(t.getMessage(), t);
      }
   ...
   */
  public HiveParserImpl(final TokenStream input) {
    super(input);
  ...
  public SqlRootContext getRootContext() {
    final HiveParserListener hiveParserListener = new HiveParserListenerImpl();

    this.removeParseListeners();
    this.addParseListener(hiveParserListener);
    this.statement();

    return hiveParserListener.getRootContext();
  }
}
