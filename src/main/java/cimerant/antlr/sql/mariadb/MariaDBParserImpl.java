...
public class MariaDBParserImpl extends MariaDBParser {
   ...
   */
  public static MariaDBParserImpl getInstance(final File file) throws IOException {
    final var moduleCode = ModuleCode.ERR_M2700;

    try {
      final var lexer = new MariaDBLexer(CharStreams.fromFileName(file.getCanonicalPath()));
      lexer.removeErrorListeners();
      lexer.addErrorListener(new ProxyErrorListenerImpl(lexer.getErrorListeners()));

      return new MariaDBParserImpl(new CommonTokenStream(lexer));
    } catch (final SysError s) {
      throw s;
    } catch (final Throwable t) {
      // 0001 | Unknown error
      if (MariaDBParserImpl.logger.isDebugEnabled()) {
        MariaDBParserImpl.logger.debug(t.getMessage(), t);
      }
   ...
   */
  public MariaDBParserImpl(final TokenStream input) {
    super(input);
  ...
  public SqlRootContext getRootContext() {
    final MariaDBParserListener mariaDBParserListener = new MariaDBParserListenerImpl();

    this.removeParseListeners();
    this.addParseListener(mariaDBParserListener);
    this.root();

    return mariaDBParserListener.getRootContext();
  }
}
